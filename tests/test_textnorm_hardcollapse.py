...
def test_hard_collapse_normalization_feeds_sentence_split() -> None:
    sample = "第一段\t\u3000A\u00A0B。\n\n第二段\u00A0继续？"
    cfg = TextNormConfig(collapse_lines=False, hard_collapse_lines=True)
    normalized = normalize_text_for_export(sample, _blank_char_map(), cfg)
    assert normalized == "第一段 A B。\n第二段继续？"
    split_cfg = TextNormConfig(hard_collapse_lines=True, split_mode="punct+len")
    lines = split_sentences_with_rules(normalized, split_cfg)
    assert lines == ["第一段 A B。", "第二段继续？"]
