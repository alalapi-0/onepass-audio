...
依赖: Python 标准库；内部类型 ``onepass.types.EDLAction``。
...
from .types import EDLAction, Word
...
def build_edl(words: list[Word], retake_cuts: list[tuple[float, float]], cfg: dict) -> list[EDLAction]:
    """根据词时间戳与重录区间构建剪辑动作列表。"""

    actions: list[EDLAction] = []
    for start, end in retake_cuts:
