...
try {
    ...
    if (-not $localItems) {
        Write-Host "No audio files found under '$LOCAL_AUDIO'."
        exit 1
    }
    ...
    if ($shouldUseRsync) {
        ...
        $items = Get-ChildItem -LiteralPath $LOCAL_AUDIO -Force
        if (-not $items) {
            Write-Host "No entries to transfer from '$LOCAL_AUDIO'."
            exit 1
        }
        $totalBytes = ($localItems | Measure-Object -Property Length -Sum).Sum
        $fileCount = $localItems.Count
        Write-Host "Will transfer $fileCount files (~$([math]::Round($totalBytes/1MB,2)) MiB) via scp."
        if ($DryRun) {
            foreach ($item in $items) {
                $source = $item.FullName
                $dest = "$VPS_USER@$VPS_HOST:$REMOTE_AUDIO/"
                Write-Host "scp -i $VPS_SSH_KEY -r -C `"$source`" `"$dest`""
            }
            Write-Host 'Dry run requested; exiting before execution.'
            exit 0
        }
        foreach ($item in $items) {
            $source = $item.FullName
            $dest = "$VPS_USER@$VPS_HOST:$REMOTE_AUDIO/"
            $scpArgs = @('-i', $VPS_SSH_KEY, '-r', '-C', '-p', $source, $dest)
            & $scpExe @scpArgs
